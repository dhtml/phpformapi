...
class phpformapi {
public static $themeapi_name1;
public static $themeapi_name2;
public static $themeapi_name3;
...
self::$themeapi_value2=$themeapi_value2;

if((isset($_POST[$themeapi_name1]))&&($_POST[$themeapi_name2]==$themeapi_value2)) {
$form=$_POST[$themeapi_name1];
self::execHandler($form,$_POST,'post');
} 
else if((isset($_GET[$themeapi_name1]))&&($_GET[$themeapi_name2]==$themeapi_value2)) {
$form=$_GET[$themeapi_name1];
self::execHandler($form,$_GET,'get');
}
...
unset($vars[self::$themeapi_name3]);
self::$currentForm=$form; //saves the currently detected form globally

if(is_array($validation)&&!empty($validation)) {
//automatic validation
self::validateForm($validation,$vars,$form,$method);
} else {
$validation=Array();
}
...
//we cannot even process results now that there are still errors
if(phpformapi::getErrors($form)) {return;}


//submit function
$action="phpformapi_submit_{$form}";
if(function_exists($action)) {
@eval ("$action".'($vars,$form,$method);');
}
...
if(!is_array(self::$errorFields)) {self::$errorFields=Array();} 

//$debug=self::jdecode($debug);
foreach($validation as $field=>$data) {
$debug=self::jdecode($data);
if(!is_array($debug)) {continue;} //the json package was not well formatted
$value=$vars[$field];

if(!self::validateFormField($field,$value,$debug)) {self::$errorFields[]=$field;}
}
...
//first thing first, let us check for required.
if(($required=='true'||$required==true)&&(empty($value)&&($required!='false'||$required==false))) {
return self::setFormError("The $title field is required.",$message);
}
...
return self::setFormError("The $title field must contain a valid email address.",$message);
}
break;

case "alphanumeric":
if(!preg_match("/^([a-z0-9])+$/i",$value)) {
return self::setFormError("The $title field must contain only alpha-numeric characters.",$message);
}
break;
...
case "number":
if(!is_numeric($value)) {
return self::setFormError("The $title field must contain a number.",$message);
}
break;

case "int":
if(!is_int($value)) {
return self::setFormError("The $title field must contain an integer.",$message);
}
break;

case "float":
if(!is_float($value)) {
return self::setFormError("The $title field must contain a floating number.",$message);
}
break;
...
//followed by maxlength
if(isset($maxlength)) {
$len=(int) $maxlength;
 if(strlen($value)>$len) {
 $x= $len==1 ? "character" : "characters";
   return self::setFormError("The $title field should not exceed $maxlength $x.",$message);
  }
}

//and then minimum length
if(isset($minlength)) {
$len=(int) $minlength;
 if(strlen($value)<$len) {
 $x= $len==1 ? "character" : "characters";
   return self::setFormError("The $title field should be at least $minlength $x.",$message);
  }
}

//and then exact length
if(isset($exactlength)) {
$len=(int) $exactlength;
 if(strlen($value)!=$len) {
 $x= $len==1 ? "character" : "characters";
   return self::setFormError("The $title field should be exactly $exactlength $x.",$message);
  }
}

//matches another field
if(isset($matches)) {
$value2=self::$requestVars[$matches];
if($value!=$value2) {
$debug=self::jdecode(self::$validation[$matches]);
$title2=$debug['title'];
...
$html = str_get_html($formHTML);
$frm=$html->find('form', 0);
if($frm=="") {return "form tag is missing.";}

//get name of form or ID as alternative
$form=$frm->getAttribute('name');
if(empty($form)) {$form=$frm->getAttribute('id');}
if(empty($form)) {return "Your form must have a name or an ID.";}

$method=trim(strtolower($frm->getAttribute('method')));
$formprops['method']= $method=="post" ? "post" : "get";

//decide on form variables
...
$frm->action = $action;
$formprops['validate']=true; //we can validate this form
} else {
$formprops['validate']=false; //this submits externally, so no validation
}
...
$t=trim(strtolower($e->getAttribute('type'))); //type
$ename=$e->getAttribute('name');
$leavealone=isset($e->leavealone);
$readonly=isset($e->readonly);

//the 3 conditions we cannot tolerate
if($leavealone&&$readonly||empty($ename)) {continue;}

//cache validation information
$valid=$e->getAttribute('phpformapi');
if(!empty($valid)) {$fields["$ename"]=$valid;}

//set error class if this field failed validation earlier
if(self::hasFailedValidation($form,$ename)) {
$cls=$e->getAttribute('class');
$cls=trim("$cls phpformapierror");
$e->class = $cls;
}
...
switch($t) {
case "text":
$e->value = $formvars["$ename"];
break;

case "checkbox":
if(isset($e->value)&&(substr_count($ename, '[]')==1)) {
$_name=substr($ename,0,strlen($ename)-2);
$clist=$formvars[$_name];
//echo "<pre>";var_dump($clist);echo "</pre>";
if (is_array($clist)&&in_array($e->value, $clist)) {
$e->checked="checked";
}

} else if((!isset($e->value)&&$formvars["$ename"]=='on')||(isset($e->value)&&$e->value==$formvars["$ename"])) {
$e->checked="checked";
}
break;
case "radio":
if(isset($e->value)&&$e->value==$formvars["$ename"]) {
$e->checked="checked";
}
break;
...
//submitted value for this selec
$selectedval=$formvars["$ename"];

//select options present inside this select
foreach($e->find('option') as $opt) { 
$opt->selected = null; //remove selected attribute first

$option_value=$opt->innertext;
$option_text=$opt->innertext;

if(!empty($option_value)&&$option_value==$selectedval) {
$opt->selected = "selected";
}
else if(!empty($option_text)&&$option_text==$selectedval) {
$opt->selected = "selected";
}
...
//add the hidden elements - this alters dome and so must come last
//$frm->innertext="\n<input name=\"phpformapi\" value=\"{$form}\" type=\"hidden\">\n".$frm->innertext;

if($formprops['validate']) {
$themeapi_name1=self::$themeapi_name1;$themeapi_name2=self::$themeapi_name2;$themeapi_name3=self::$themeapi_name3;
$themeapi_value2=self::$themeapi_value2;
$validation_code=base64_encode(serialize($fields));

$formapitag=<<<end
<!--phpformapi tags start-->
<input name="$themeapi_name1" value="{$form}" type="hidden">
<input name="$themeapi_name2" value="{$themeapi_value2}" type="hidden">
<textarea style="display:none" name="$themeapi_name3">$validation_code</textarea>
<!--phpformapi tags stop-->
end;
$html->find('form', 0)->innertext=$formapitag.$html->find('form', 0)->innertext;
}
